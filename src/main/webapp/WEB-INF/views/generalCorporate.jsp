...
$(document).ready(function(){
	  ...
	  var data = { seqno: $('#seqno').val(), 
	  			   ...
	  			   xmlnum: $('#xmlnum').val() };
	  $.post("/generalCorporate/preview", data,function(data) {
		  ...
		  //循环取返回list的值，并用jquery 操作table插入进去
		  	data.forEach(function(val,index,arr){
		  		...
		  		+val['clientname']+"</td><td>"+val['idtype']+"</td><td>"
		  		+val['nocid']+"</td><td>"+val['classify']+"</td><td>"+val['option_id']+"</td></tr>";
		  		//在表格最后插入新行
...
<a href="/assetMgrClient">资管客户</a>
		...
		<form target="down-iframe" method="post" action="${ctx}/generalCorporate/download">
			<fieldset>
				<table border="1" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td>客户类型：</td>
						<td><input type="text" id="clienttype" name="clienttype" readonly="readonly"  class="text1" value="2"/></td>
						<td>客户名称：</td>
					...
					<tr>
						<td>证件类型：</td>
						    <td> <select name="idtype" id="idtype" style="width: 400px; height: 30px"  onchange="check(this)">
							 <option value="50">50（组织机构代码证）</option>
    							 <option value="51">51（商业登记证）</option>
    							</select>
    					   </td>	
						<td>组织机构代码：</td>
					...
					<tr>
						...
						<td>投资者类型：</td>
						<td><input type="text" id="classify" name="classify" class="text1" readonly="readonly" value="02"/></td>
					</tr>
					<tr>
						<td>开户数：</td>
