...
$(document).ready(function(){
	  ...
	  var data = { seqno: $('#seqno').val(), 
			  	   ...
			  	   exclientidrefname: $('#exclientidrefname').val(), 
				   clientname: $('#clientname').val(), 
				   clientname_eng: $('#clientname_eng').val(), 				   
			  	   nocid: $('#nocid').val(),
			  	   nocid_extracode: $('#nocid_extracode').val(),
			  	   classify: $('#classify').val(),
			  	   department_code: $('#department_code').val(),			  	   			  	   
			  	   clientnum: $('#clientnum').val(), 
	  			   xmlnum: $('#xmlnum').val() };
	  $.post("/specialCorporate/preview", data,function(data) {
		  ...
		  //循环取返回list的值，并用jquery 操作table插入进去
		  	data.forEach(function(val,index,arr){
		  		...
		  		+val['exclientidrefname']+"</td><td>"+val['clientname']+"</td><td>"+val['clientname_eng']+"</td><td>"
		  		+val['nocid']+"</td><td>"+val['nocid_extracode']+"</td><td>"+val['classify']+"</td><td>"+val['department_code']+"</td></tr>";
		  		//在表格最后插入新行
...
<a href="/assetMgrClient">资管客户</a>
		...
		<form target="down-iframe" method="post" action="${ctx}/specialCorporate/download">
			<fieldset>
				<table border="1" cellspacing="0" cellpadding="0"
					...
					<tr>
						<td>客户类型：</td>
						<td><input type="text" id="clienttype" name="clienttype" readonly="readonly"  class="text1" value="4"/></td>
						<td>会员号：</td>
						<td><input type="text" id="excompanyid" name="excompanyid" class="text1" /></td>	
					</tr>
					<tr>
						<td>交易编码名称：</td>
						<td><input type="text" id="exclientidrefname" name="exclientidrefname" class="text1"/></td>
						<td>交易类型：</td>
					...
					<tr>
						...
						<td>英文名称：</td>
						<td><input type="text" id="clientname_eng" name="clientname_eng" class="text1" /></td>	
					</tr>
					<tr>
						<td>组织机构代码：</td>
						<td><input type="text" id="nocid" name="nocid" class="text1"/></td>
						<td>附加码：</td>
						<td><input type="text" id="nocid_extracode" name="nocid_extracode" class="text1" /></td>	
					</tr>
					<tr>
                       <td>投资者类型：</td>
						  <td><input type="text" id="classify" name="classify" class="text1" readonly="readonly" value="03"/></td>				
					   <td>营业部代码：</td>
						  <td><input type="text" id="department_code" name="department_code" class="text1" /></td>
					</tr>
					<tr>
						<td>开户数：</td>
		...
		<tr>
			...
			<th>exclientid</th>
			<th>exclientidrefname</th>
			<th>clientname</th>
			<th>clientname_eng</th>
			<th>nocid</th>
			<th>nocid_extracode</th>
			<th>classify</th>
